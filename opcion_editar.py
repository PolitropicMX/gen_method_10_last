...
def editar(root): # AQUI INICIA EDICION DE UN METODO
    ...
    def save_method():
        ...
        # Se deben guardar los cambios de los entry's
        nombre = name_prod_entry.get()
        noinv = no_prod_entry.get()
        picto = picto_prod_entry.get()
        envprod = etiq_prod_entry.get()
        envmues = mues_prod_entry.get()
        equipo = equi_labe_entry.get()
        malla = mall_labe_entry.get()
##        print()
        print(f" envase del producto es: {envprod} para control es: {envmues}")
        datos_cargados[nombre_producto]["Informacion"]["no inv"] = noinv
        datos_cargados[nombre_producto]["Informacion"]["pictogramas"] = picto
        if malla == "":
                datos_cargados[nombre_producto]["Envase"] =  {"malla":"no"}
        else:
                datos_cargados[nombre_producto]["Envase"] =  {"malla":f"{malla}"}
        datos_cargados[nombre_producto]["Envase"]["Envase_producto"] = envprod
        datos_cargados[nombre_producto]["Envase"]["Envase_control"] = envmues
        datos_cargados[nombre_producto]["Equipo a utilizar"]["Equipo a utilizar"] = equipo
        print(f"Los datos guardados quedan como: \n{datos_cargados[nombre_producto]}")
    ...
    def option_selected(event): # ESTA FUNCION ES PARA EL COMBOBOX: REACTIVO, PASO, BANNER
        ...
        mall_labe_entry.delete(0,tk.END)
            # POR CADA PRODUCTO EN PRODUCTOS
        if selected_option in (datos_cargados):
            ...
            for k,data in enumerate(datos_cargados[selected_option]):
                ...
                if data == "Informacion":
                    ...
                    if "Envase_producto" in datos_cargados[selected_option]["Envase"]:
                        etiq_prod_entry.insert(0,datos_cargados[selected_option]["Envase"]["Envase_producto"])
                    if "Envase_control" in datos_cargados[selected_option]["Envase"]:
                        mues_prod_entry.insert(0,datos_cargados[selected_option]["Envase"]["Envase_control"])
                    if "malla" in datos_cargados[selected_option]["Envase"]:
                        mall_labe_entry.insert(0,datos_cargados[selected_option]["Envase"]["malla"])
    ...
    def editar_bloque():
        ...
        def tipo_bloque(bloque):
            ...
            if bloque[0] == "1" or bloque[0] == "2":# Es REACTIVO
                ...
                # BUTTONS ----------------------------------------------------------------
                guardar = ttk.Button(botones_edit,text="Guardar",command=save)
                guardar.grid(row=0, column=0,padx=5,pady=5)
                guardar.bind("<Return>", on_enter)
                cerrar =  ttk.Button(botones_edit,text="Cerrar",command=cerrar)
                cerrar.grid(row=0, column=1,padx=5,pady=5)
                cerrar.bind("<Return>", on_enter)
        ...
        if len(selected_item) == 0:
            messagebox.showwarning("Advertencia", f"No has seleccionado ningun bloque para editar")
        else:
...
##                    print(f'Item#{i} : {item}')
            ...
            tk.Button(option_frames[lista_opciones[0]],text="agregar",command=agregar_reactivo).grid(row=row+len(specs), column=0).bind("<Return>", on_enter)
            # --COMBOBOX------------------------------------------------OPCION 2-----------------------------------------------------------------------------------------
            ...
            tk.Button(option_frames[lista_opciones[1]],text="agregar",command=agregar_paso).grid(row=row+len(specs), column=0).bind("<Return>", on_enter)
            # --COMBOBOX------------------------------------------------OPCION 3-----------------------------------------------------------------------------------------
            ...
            # BOTON QUE AGREGA EL TEXTO AL TREEVIEW
            qwe = tk.Button(option_frames[lista_opciones[2]],text="agregar",command=agregar_banner)
            qwe.grid(row=row+len(specs), column=0)
            qwe.bind("<Return>", on_enter)
            # BOTON QUE AGREGA NUEVO BANNER
            wer = ttk.Button(option_frames[lista_opciones[2]],text="agregar",command=agregar_n_banner)
            wer.grid(row=row+len(specs), column=1)
            wer.bind("<Return>", on_enter)
    # -------------------------------------------------------------------------------------- FRAMES ----------------------------------------------------------------
    third_window = tk.Toplevel(root,bg="#776B5D")
    third_window.title("Editar metodo de Fabricación")
    third_window.focus_force()
    data_n_tree = tk.Frame(third_window,bg="#EBE3D5") # 1 (0,0)
    ...
    d1 = tk.LabelFrame(datos,text="Datos del producto",bg="#EBE3D5")# 3 (0,0)
    ...
    d4 = tk.LabelFrame(d1,text="Equipo a utilizar",bg="#B0A695")# 4 (6,0)
    d4.grid(row=6,column=0)
##    d4.configure(bg="#ffffff")
...
##    productos_reactivos = Productos()
    ...
    # ETIQUETA
    name_prod = tk.Label(d1, text="Nombre del productos", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    ...
    # ----------------------------------------------------------------------
        # NO DE INVENTARIO
    # ETIQUETA
    no_prod = tk.Label(d1, text="No. Inv.", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    ...
    # ETIQUETA
    picto_prod = tk.Label(d1, text="Str pictogramas max 3", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    ...
    # ----------------------------------------------------------------------
        # ENVASE 
    # ETIQUETA
    etiq_prod = tk.Label(d1, text="Envasar en: ", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    ...
    # ----------------------------------------------------------------------
        # PASAR MUESTRA EN 
    # ETIQUETA
    mues_prod = tk.Label(d1, text="Pasar Muestra en: ", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    ...
    # ----------------------------------------------------------------------
        # EQUIPO A UTILIZAR
    # ETIQUETA
    equi_labe = tk.Label(d1, text="Equipo a utilizar: ", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    ...
    # ----------------------------------------------------------------------
        # MALLA
    # ETIQUETA
    mall_labe = tk.Label(d1, text="Malla a utilizar: ", font=('Arial', 9),bg="#EBE3D5",fg="#000000")
    mall_labe.grid(row=7, column=0)
    # ENTRY                    
    mall_labe_entry = ttk.Entry(d1,font=("Helvetica",15))
    mall_labe_entry.grid(row=7, column=1)
    # --------------------------------------------------------------BOTONES DE EDICION-----------------------------------------------------------------
    edit_boton = tk.Button(botones,text="Editar bloque",command=editar_bloque,bg="#FFD6A5")
    edit_boton.grid(row=0, column=0)
    edit_boton.bind("<Return>", on_enter)
    dele_boton = tk.Button(botones,text="Eliminar bloque",command = eliminar_bloque,bg="#FF9B9B")
    dele_boton.grid(row=0, column=1)
    dele_boton.bind("<Return>", on_enter)
    add_boton = tk.Button(botones,text="añadir bloque",command=anadir_bloque,bg="#CBFFA9")
    add_boton.grid(row=0, column=2)
    add_boton.bind("<Return>", on_enter)
    guar_boton = tk.Button(botones,text="Guardar metodo",command=save_method,bg="Lightblue")
    guar_boton.grid(row=0, column=3)
    guar_boton.bind("<Return>", on_enter)
    # --------------------------------------------------------------TREEVIEW-----------------------------------------------------------------------------
    ...
    third_window.update()
    third_window.minsize(third_window.winfo_width(), third_window.winfo_height())
    x_cordinate = int((third_window.winfo_screenwidth()/2) - (third_window.winfo_width()/2))
    y_cordinate = int((third_window.winfo_screenheight()/2) - (third_window.winfo_height()/2))
    third_window.geometry("+{}+{}".format(x_cordinate, y_cordinate))
