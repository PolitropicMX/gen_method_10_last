...
class Save_module:
    ...
    def active_module(self,nombre,Noinv,dicc,autor):
        ...
        #       DISEÑO DE LA PAGINA
        # Margenes
        margenes = [0.5,0.5,0.5,1,0.3,0.3]
        self.ws.page_margins = PageMargins(left=margenes[0], right=margenes[1], top=margenes[2], bottom=margenes[3], header=margenes[4], footer=margenes[5])
...
##            print(f'{i}: {producto}')
        ...
        if op2 in datos_cargados:# SE COMPRUEBA SI EL STRING INGRESADO ESTA EN EL DICCIONARIO
            ...
            if "malla" in datos_cargados[op2]["Envase"]:
                print("Si hay campo 'malla'")
                self.malla = datos_cargados[op2]["Envase"]["malla"]
                print(self.malla)
                if self.malla == "no":
                    haymalla = "no"
                    descripcionmalla = ""
                    ...
                    haymalla = "si"
                    descripcionmalla = f" Envase filtrando su material con una malla de {self.malla} y envase con agitación"
            ...
            for i,reactivo in enumerate(datos_cargados[str(op2)].keys()):
                ...
                if i != len(datos_cargados[str(op2)].keys())-1 and i != len(datos_cargados[str(op2)].keys())-2:
                    ...
                    if reactivo[:4] == "Paso":# LOS PRIMEROS 4 CARACTERES
                        ...
                        # HOT NEW    
                        if datos_cargados[op2][f'Banner {n_banner}'] in images:
                            ruta = images[datos_cargados[op2][f'Banner {n_banner}']]
                        else:
                            ruta = datos_cargados[op2][f'Banner {n_banner}']
##                            print("VAs bien")
##                        else:
...
##                    print(f'anadir es {anadir[0]} y su valor {anadir[1]}')
        ...
        ####################################################################################################################################################3
            ...
            parte_inferior = [['A'+str(fila),'TOMA DE MUESTRA'],
                              ...
                              ['D'+str(fila),'ENVASADO'],
                              ['D'+str(fila+1),f'1) Este Material {haymalla} se filtra'],
                              ...
                              ['F'+str(fila+1),f'Una vez aprobado su producto proceda a envasar en {self.envase_producto}.{descripcionmalla}'],
            ...
            for fila in self.ws[f'A1:F{fila+3}']:
                for celda in fila:
                    celda.border = border_style
            # ---------------------------------------------------------------------------
                        
            self.reactivos_hoja =  []
# GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG AQUI ES CUANDO NO SE ENCUENTRA EL PRODUCTO (NO MODIFICAR) GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG
            ...
            if dicc == "0":
                ...
                if "malla" in datos_cargados[op2]["Envase"]:
                    ...
                    if self.malla == "no":
                        ...
                        haymalla = "si"
                        descripcionmalla = " Envase filtrando su material con una malla de {self.malla} y envase con agitación"
                    # Se itera a traves de el producto seleccionado
                    # reactivo es la llave del nivel 2
                    # {key1: value1}
                    # {Key1: {Key2:Value2}}
